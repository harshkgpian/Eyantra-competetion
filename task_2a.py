...
def transform_vision_sensor_depth_image(vision_sensor_depth_image, image_resolution):
	...
	#transformed_depth_image=cv2.cvtColor(transformed_depth_image, cv2.COLOR_BGR2RGB)
...
def detect_berries(transformed_image, transformed_depth_image):
	...
	if lb<0:
		lb=0
	_,threshold = cv2.threshold(gray, lb, ub, cv2.THRESH_BINARY)
	...
	for contour in contours:
		...
		if M['m00']!=0.0:
			x=int(M['m10']/M['m00'])
			y=int(M['m01']/M['m00'])
		if(img[y][x][2]>220 and img[y][x][1]<11 and img[y][x][0]>65):
			color="Red"
			strawberries.append((x,y,transformed_depth_image[y][x]))
		elif(img[y][x][0]>220 and img[y][x][1]>65 and img[y][x][2]<11):
			color="Blue"
			blueberries.append((x,y,transformed_depth_image[y][x]))
		elif(img[y][x][2]>220 and img[y][x][1]>220 and img[y][x][0]<11):
			color="Yellow"
			lemon.append((x,y,transformed_depth_image[y][x]))
		berries_dictionary[berries[0]]=strawberries
		berries_dictionary[berries[1]]=blueberries
		berries_dictionary[berries[2]]=lemon
	##################################################
...
def detect_berry_positions(berries_dictionary):
	...
	for i in p:
		...
		for j in range(len(p[i])):
			x=float("{:.2f}".format((p[i][j][0])/512-0.5))
			y=float("{:.2f}".format((p[i][j][1])/512-0.5))
			d=float("{:.2f}".format((p[i][j][2])*2+0.03))
			#g=(d*d)-((x*x)+(y*y))
			#d1=math.sqrt(g)
			pos.append((x,d,-1*y))
		berry_positions_dictionary[i]=pos
	##################################################
